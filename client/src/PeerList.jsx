...
const PeerList = ({ socket, name }) => {
    ...
    return (
        <div className="dark:bg-slate-800 flex flex-col h-screen p-6 rounded-md shadow-2xl items-center space-x-1">
            <h3 className="text-3xl text-emerald-500 dark:text-emerald-200 font-semibold hover:text-emerald-400">You are known as {localStorage.getItem('name')}</h3>
            <ul className="flex flex-row m-7">
                {list.map((number) => (
                    <div className="parent flex flex-col justify-center items-center">
                        ...
                        <p className="m-3 text-md font-bold dark:text-slate-300">{number}</p>
                    </div>
                ))}
            </ul>
            <input multiple="multiple" onChange={fileWatch} ref={inputFile} type="file" style={{ display: "none" }} accept="image/*" />
            {sourceImg.length != 0 ? <p className="dark:text-slate-300">{sourceImg.length} images are coming your way!</p> : "No Images"}
